...
\includegraphics[width=0.975\textwidth,height=0.691\textwidth]{figure/qboundhaircutting-1} \caption{Q-Q plot of $10^{6}$ simulated \txtQual values versus \apxref{qual_dist} is shown. Units are `annual', \ie \yrtomhalf. Since the number of samples is very large, only a subset of $10^{3}$ points, uniformly selected by sample quantile, are plotted.}\label{fig:haircutting}
\end{figure}
...
$\ensuremath{10^{6}}$ Monte Carlo simulations were conducted for 
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[h]
\includegraphics[width=0.99\textwidth,height=0.495\textwidth]{figure/qboundksheat-1} \caption[The \txtKS statistic for \apxref{qual_dist} over $10^{6}$ simulations of Gaussian returns is indicated, by color, versus \nlatf, and the `total effect size,' $\sqrt{\ssiz}\psnropt$, which is a unitless quantity]{The \txtKS statistic for \apxref{qual_dist} over $10^{6}$ simulations of Gaussian returns is indicated, by color, versus \nlatf, and the `total effect size,' $\sqrt{\ssiz}\psnropt$, which is a unitless quantity. Different facets are for different values of \ssiz (in years).}\label{fig:ksheat}
\end{figure}
...
$\psnropt=0.86\yrtomhalf$. The bound on expected
value of \pql{\sportwfnc{\mreti}} from \theoremref{qual_bound}
is only $0.46\yrtomhalf$.  Under \apxref{qual_dist}, 
the probability that \pql{\sportwfnc{\mreti}} exceeds 
$0.67\yrtomhalf$ in this case is only
$0.1$. It is not surprising that DeMiguel \etal drew
