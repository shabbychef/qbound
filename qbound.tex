...
\title{Bounds on Portfolio Quality}
\author{Steven E. Pav \thanks{\email{steven@gilgamath.com} The author thanks
Ramakrishna Kakarala for sharing his research.
...
\spherep.  When the portfolio is built using noisy data, the expected
value of the \txtQual is bounded from above via a \txtCR bound, 
%on the portfolio estimator, 
for the case of Gaussian returns. The bound holds
for `biased' estimators, thus there appears to be no bias-variance
tradeoff for the problem of maximizing the \txtQual. 
An approximate distribution of the \txtQual for the \txtMP is 
given, and shown to be fairly accurate via Monte Carlo simulations, 
for Gaussian returns as well as more exotic returns distributions.
...
Monte Carlo simulations: $\ensuremath{10^{6}}$ simulations were performed
of construction of the \txtMP 
using $\ssiz=1012$ (4 years of daily observations), 
$\nlatf=6$ and $\psnropt=1.25\yrtomhalf$;
...
theoretical CDF under the approximation, was computed to be 0.005
over the $\ensuremath{10^{6}}$ simulations.
While this seems small, the computed p-value under the null underflows to 0
...
\includegraphics[width=\maxwidth]{figure/qboundhaircutting-1} \caption{Q-Q plot of $10^{6}$ simulated \txtQual values versus \apxref{qual_dist} is shown. Units are `annual', \ie \yrtomhalf. Since the number of samples is very large, only a subset of $10^{4}$ points, uniformly selected by sample quantile, are plotted.}\label{fig:haircutting}
\end{figure}
...
a \tstat{}-distribution with $4$ degrees of freedom, 
from a Tukey $h$-distribution with parameter $h=0.15$, 
and from a Lambert W $\times$ Gaussian
with parameter $\gamma=-0.2$.  \cite{2009arXiv0912.4554G,2010arXiv1010.2265G}
...
Again, $\ensuremath{10^{6}}$ simulations are performed under each of these 
...
% Wed May  9 22:25:17 2018
\begin{table}[ht]
\centering
\begin{tabular}{r|ccccc|c}
  \hline
q.tile & normal & unif. & t(4) & Tukey(0.15) & Lam.W(-0.2) & approx. \\ 
  \hline
0.005 & -0.0443 & -0.0509 & -0.0443 & -0.0412 & -0.0453 & -0.0450 \\ 
  0.010 & 0.0995 & 0.0911 & 0.0973 & 0.0980 & 0.0965 & 0.0996 \\ 
  0.025 & 0.2891 & 0.2867 & 0.2907 & 0.2915 & 0.2880 & 0.2928 \\ 
  0.050 & 0.4367 & 0.4345 & 0.4375 & 0.4377 & 0.4355 & 0.4397 \\ 
  0.250 & 0.7860 & 0.7854 & 0.7872 & 0.7862 & 0.7854 & 0.7890 \\ 
  0.500 & 0.9527 & 0.9525 & 0.9532 & 0.9527 & 0.9522 & 0.9550 \\ 
  0.750 & 1.0705 & 1.0706 & 1.0707 & 1.0705 & 1.0701 & 1.0721 \\ 
  0.900 & 1.1430 & 1.1431 & 1.1435 & 1.1429 & 1.1428 & 1.1442 \\ 
   \hline
\end{tabular}
\caption{Empirical quantiles of portfolio \txtQual from $10^{6}$ simulations of 1012 days of 6 assets, with maximal \txtSR of $1.25\yrtomhalf$ are given, along with the approximate quantiles from \apxref{qual_dist}. Units of \txtQual are `annual', \ie \yrtomhalf.} 
\label{tab:qtiles}
\end{table}
...
normal & unif. & t(4) & Tukey(0.15) & Lam.W(-0.2) \\ 
  \hline
0.0046 & 0.0055 & 0.0037 & 0.0047 & 0.0058 \\ 
   \hline
\end{tabular}
\caption{\txtKS statistic comparing the empirical CDF to that of \apxref{qual_dist} over $10^{6}$ simulations of 1012 days of 6 assets, with maximal \txtSR of $1.25\yrtomhalf$ are given for the different returns distributions.} 
\label{tab:ks_stats}
\end{table}
...
\begin{tabular}{ccccc|c}
  \hline
normal & unif. & t(4) & Tukey(0.15) & Lam.W(-0.2) & bound \\ 
  \hline
0.9 & 0.898 & 0.899 & 0.899 & 0.898 & 0.932 \\ 
   \hline
\end{tabular}
\caption{Empirical mean portfolio \txtQual from $10^{6}$ simulations of 1012 days of 6 assets, with maximal \txtSR of $1.25\yrtomhalf$ are given, along with the upper bound from \theoremref{qual_bound}. Units of \txtQual are `annual', \ie \yrtomhalf.} 
\label{tab:means}
\end{table}
% latex table generated in R 3.3.2 by xtable 1.8-2 package
% Wed May  9 22:25:17 2018
...
0.9 & 0.864 & 0.865 & 0.864 & 0.863 & 0.868 \\ 
   \hline
\end{tabular}
\caption{Empirical mean of \emph{squared} portfolio \txtQual from $10^{6}$ simulations of 1012 days of 6 assets, with maximal \txtSR of $1.25\yrtomhalf$ are given, along with the approximate value from \eqnref{hypergeo_extwo}. Units of squared \txtQual are `annual', \ie \yrto{-1}.} 
\label{tab:meanssq}
\end{table}


In \tabref{means}, the empirical mean value of \pql{\sportwopt}, over
the $\ensuremath{10^{6}}$ simulations, is presented for the five returns
...
$\ensuremath{10^{5}}$ Monte Carlo simulations were conducted for 
each combination of $\ssiz=0.5, 1, 2, 4, 8$ years of daily observations,
$\nlatf=2, 4, 8, 16$, and
$\psnropt=0.35, 0.5, 0.71, 1, 1.41\yrtomhalf$,
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/qboundksplots-1} \caption{The \txtKS statistic for \apxref{qual_dist} over $10^{5}$ simulations of Gaussian returns is plotted versus $\psnropt \wrapParens{\nlatf-1}/\sqrt{\ssiz}$, with \psnropt in annualized terms, and \ssiz measured in years.  There is one line for each combination of $\ssiz$ and $\psnropt$. The line color corresponds to the `effect size', $\sqrt{\ssiz}\psnropt$, which is unitless.}\label{fig:ksplots}
\end{figure}
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/qboundksheat-1} \caption[The \txtKS statistic for \apxref{qual_dist} over $10^{5}$ simulations of Gaussian returns is indicated, by color, versus \nlatf, and the `total effect size,' $\sqrt{\ssiz}\psnropt$, which is a unitless quantity]{The \txtKS statistic for \apxref{qual_dist} over $10^{5}$ simulations of Gaussian returns is indicated, by color, versus \nlatf, and the `total effect size,' $\sqrt{\ssiz}\psnropt$, which is a unitless quantity. Different facets are for different values of \ssiz (in years).}\label{fig:ksheat}
\end{figure}
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/qboundgrow_bound-1} \caption{The upper bound of \theoremref{qual_bound} is plotted versus \nlatf for different scaling laws for \psnropt.  These scaling laws correspond to $\psnropt = \psnr[0]\nlatf^{\gamma}$, with $\gamma$ taking values between 0.15 and
0.35.  The constant terms, \psnr[0], are adjusted so that $\psnropt =1.25\yrtomhalf$ for $\nlatf = 6$ for all the lines. The bound uses $\ssiz=1012$, corresponding to 4 years of daily observations.}\label{fig:grow_bound}
\end{figure}
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/qboundgrow_sqrt-1} \caption[Some quantiles of the \txtQual of the \txtMP, under \apxref{qual_dist}, are plotted versus \nlatf for different scaling laws for \psnropt]{Some quantiles of the \txtQual of the \txtMP, under \apxref{qual_dist}, are plotted versus \nlatf for different scaling laws for \psnropt. The 3 panels represent different values of $\gamma$, \viz 0.21, 0.25, and 0.29. The bound uses $\ssiz=1012$, corresponding to 4 years of daily observations.}\label{fig:grow_sqrt}
\end{figure}
...
assets, over the 1080 months, is around 
$0.66\yrtomhalf$. The \txtSR of the sample \txtMP over
the 10 assets over the same period is around
$0.89\yrtomhalf$.  \cite{SharpeR-Manual}
Now consider a portfolio estimator
given 5 years of observations, as in 
DeMiguel \etal \cite{demiguel2009optimal}, assuming 
$\psnropt=0.89\yrtomhalf$. The bound on expected
value of \pql{\sportwfnc{\mreti}} from \theoremref{qual_bound}
is only $0.49\yrtomhalf$.  Under \apxref{qual_dist}, 
the probability that \pql{\sportwfnc{\mreti}} exceeds 
$0.66\yrtomhalf$ in this case is only
$0.16$. It is not surprising that DeMiguel \etal drew
...
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/qboundsp100_box-1} \caption[Growth of estimated \psnropt versus \nlatf for the S\&P 100 Index names is shown over 1000 permutations of the stocks]{Growth of estimated \psnropt versus \nlatf for the S\&P 100 Index names is shown over 1000 permutations of the stocks. There is effectively \emph{no} diversification benefit here beyond an equal weight portfolio.}\label{fig:sp100_box}
\end{figure}
