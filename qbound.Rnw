...
We characterize this observed behavior, where the bound on quality of the \txtMP is getting worse
as one adds more assets to the universe, even though the population optimal
\txtSNR is growing, as ``overfitting''.
One would get better out-of-sample performance from a portfolio which ignored 
the additional assets, effectively setting their weights to zero.
...
number of non-zero weights in ones portfolio.
...
<<'sp100_grow',dependson=c('sp100_divs','plot_stuff'),fig.width=wfg_width,fig.height=wfg_height,out.width=wot_width,out.height=wot_height,fig.cap=paste0("Growth of estimated \\psnropt versus \\nlatf for the S\\&P 100 Index names, in alphabetical order, showing the `Apple effect.'"),eval.after='fig.cap'>>= 
...
<<'sp100_box',dependson=c('sp100_divs','plot_stuff'),fig.width=wfg_width,fig.height=wfg_height,out.width=wot_width,out.height=wot_height,fig.cap=paste0("Growth of estimated \\psnropt versus \\nlatf for the S\\&P 100 Index names is shown over ",scales::comma(dim(buncho.KRSs)[2])," permutations of the stocks, showing that there is effectively \\emph{no} diversification benefit here beyond an equal weight portfolio."),eval.after='fig.cap'>>= 
...
$\chol{\pvsig}$, only increase the \txtCR bound on variance\footnote{I suspect
  this is a commonly used result, but could not find a reference.}
