...
@
<<'haircut_study',dependson=c('runtime','consts','haircut_study_setup')>>=
# set everything up
...
<<'kstestit',dependson=c('haircut_study','hcut_qtile')>>=
x <- quals.gaussian
x <- x[!is.na(x)]
...
<<'haircutting',dependson=c('haircut_study','plot_stuff'),fig.cap=paste0("Q-Q plot of $10^{",round(log10(n.sim)),"}$ simulated \\txtQual values versus \\apxref{qual_dist} is shown.")>>=
# qqplot;
...
<<'qtiles',dependson=c('haircut_study'),results='asis'>>=
...
<<'ks_table',dependson=c('kstestit'),results='asis'>>=
...
<<'means',dependson=c('haircut_study','hcut_qtile'),results='asis'>>=
...
<<'quality_heatmaps',dependson=c('runtime','consts','haircut_study_setup')>>=
suppressMessages({
  require(SharpeR)
  require(readr)
})
...
<<'ksplots',dependson=c('quality_heatmaps'),fig.width=wfg_width,fig.height=wfg_height,out.width=wot_width,out.height=wot_height,fig.cap=paste0("The \\txtKS statistic for \\apxref{qual_dist} over $10^{",round(log10(n.sim3)),"}$ simulations of Gaussian returns is plotted versus $\\psnropt \\wrapParens{\\nlatf-1}/\\sqrt{\\ssiz}$, with \\psnropt in annualized terms, and \\ssiz measured in years."),eval.after='fig.cap'>>= 
...
<<'ksheat',dependson=c('quality_heatmaps'),fig.width=wfg_width,fig.height=wfg_height,out.width=wot_width,out.height=wot_height,fig.cap=paste0("The \\txtKS statistic for \\apxref{qual_dist} over $10^{",round(log10(n.sim3)),"}$ simulations of Gaussian returns is indicated, by color, versus \\nlatf, and the `total effect size,' $\\sqrt{\\ssiz}\\psnropt$, which is a unitless quantity, with facets for different values of \\ssiz (in years)."),eval.after='fig.cap'>>= 
