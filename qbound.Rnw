...
We repeat this \Sexpr{mc_nrep} times for each setting of the parameters,
...
zallrep %>%
  ...
  labs(x=expression(zeta[`*`]),
       y=expression(hat(zeta[`*`])),
       color=expression(p),
       linetype='line:',
       title='Empirical Expected Signal Noise of Markowitz Portfolio',
       caption=glue("Using {mc_nrep} simulations per configuration. Error bars at +/- 1 standard error.\nAssumes {ope} trading days per year."))
@
...
<<'grow_bound',dependson=c('plot_stuff'),fig.width=wfg_width,fig.height=wfg_height,out.width=wot_width,out.height=wot_height,fig.cap=fig_cap,fig.scap="The upper bound of \\theoremref{qual_bound} is plotted versus \\nlatf for different scaling laws.",eval.after='fig.cap'>>= 
...
bound.plot <- function(pow,n.obs,zeta.s,n.stok,plims=c(2,250),...) {
	melt.df <- NULL
	for (ppp in pow) {
		addon.df <- bound.experiment(ppp,n.obs,zeta.s,plims=plims,n.stok=n.stok,...)
		if (is.null(melt.df))
			melt.df <- addon.df
		else
			melt.df <- rbind(melt.df,addon.df)
    ...
    labs(x="# assets",y="Signal-Noise ratio",
         caption=paste0("Using n=",n.obs,". Constant terms are set so all lines have the same SNR at p=",n.stok,"."))
	return(ph)
}
...
the theorem proceeds easily\footnote{We expect few reviewers will have made it
to this point. If you find yourself reading this, congratulations!}.
...
<<'sp100_box',dependson=c('sp100_divs','plot_stuff'),fig.width=wfg_width,fig.height=wfg_height,out.width=wot_width,out.height=wot_height,fig.cap=paste0("Growth of estimated \\psnropt versus \\nlatf for the S\\&P 100 Index names is shown over ",dim(buncho.KRSs)[2]," permutations of the stocks, showing that there is effectively \\emph{no} diversification benefit here beyond an equal weight portfolio."),eval.after='fig.cap'>>= 
...
quantitative portfolio managers.  \cite{pav2020inference}
...
\begin{align*}
  ...
  \minv{\tilde{\Mtx{W}}}\wrapParens{\psnropt\basev[1] + \vect{\tilde{z}}}}{%
    \sqrt{\minv{\tilde{\Mtx{W}}}\wrapParens{\psnropt\basev[1]
    + \vect{\tilde{z}}}}}},
\end{align*}
where where \Mtx{\tilde{W}} is a Wishart random variable with scale matrix \eye[\nlatf], 
