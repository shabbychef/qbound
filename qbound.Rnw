...
\begin{itemize}
  \item We first show that for purposes of determining \txtQual, portfolio
    ...
    sphere.
  \item We use a \txtCR bound for estimation problems on hyperspheres to 
    establish that the \emph{variance} of the estimated point is greater than
    some value. This applies to general purpose estimators, but not stopped
    clocks.
